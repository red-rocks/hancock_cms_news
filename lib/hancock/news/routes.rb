module ActionDispatch::Routing
  class Mapper
    ...
    private
    def generate_hancock_news_user_routes(routes_config)
    ...
    def generate_hancock_news_cms_routes(routes_config)
      scope module: 'hancock' do
        scope module: 'news' do
          if routes_config[:use_news_path] and !routes_config[:classes][:news].nil?
            resources routes_config[:classes][:news].to_sym, only: [:show], path: routes_config[:paths][:news], as: :hancock_news_news do
              get '(/page/:page)', action: :index, on: :collection, as: ""
            end
          end
          if routes_config[:use_categories_path] and !routes_config[:classes][:categories].nil?
            resources routes_config[:classes][:categories].to_sym, only: [:show], path: routes_config[:paths][:categories], as: :hancock_news_categories do
              get '(/page/:page)', action: :index, on: :collection, as: ""
            end
          end
        end
